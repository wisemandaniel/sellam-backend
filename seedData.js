...
// scripts/migrate-confirmedAt.js
require('dotenv').config();
const mongoose = require('mongoose');
const Order = require('./models/Order'); // adjust path

async function migrateConfirmedAt() {
  try {
    await mongoose.connect(process.env.MONGODB_URI || 'mongodb://localhost:27017/yourdb');
    console.log('Connected to database.');

    // Find all confirmed orders that do not have confirmedAt set
    const filter = {
      status: 'confirmed',
      confirmedAt: { $exists: false },
    };

    // Set confirmedAt = createdAt (or createdAt + 5 minutes if you prefer)
    const update = [
      {
        $set: {
          // confirmedAt: '$createdAt',
          // or if you want createdAt + 5 minutes:
          confirmedAt: { $add: ['$createdAt', 5 * 60 * 1000] }
        },
      },
    ];

    const result = await Order.updateMany(filter, update);
    console.log(`Updated ${result.nModified} orders with confirmedAt.`);
  } catch (error) {
